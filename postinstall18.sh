...
sudo apt -y install ssh ffmpeg vlc mc chromium-browser git gimp wget curl ssh nmap printer-driver-cups-pdf imagemagick libcanberra-gtk-module build-essential checkinstall lm-sensors make laptop-mode-tools gparted tmux vim nano yandex-browser-beta network-manager-l2tp network-manager-l2tp-gnome  openvpn easy-rsa  network-manager-openvpn-gnome network-manager-openvpn apt-transport-https
sudo apt -y install links exfat-fuse exfat-utils
